...
installbbrplus(){
	...
	if [[ "${release}" == "centos" ]]; then
		...
		kernel_version="4.14.168_bbrplus" #fix a bug
	elif [[ "${release}" == "debian" || "${release}" == "ubuntu" ]]; then
...
————————————内核管理————————————
 ${Green_font_prefix}1.${Font_color_suffix} 安装 BBR内核-5.4.14
...
————————————加速管理————————————
 ...
 ${Green_font_prefix}5.${Font_color_suffix} 使用BBR魔改版加速(可能无法使用)
 ${Green_font_prefix}6.${Font_color_suffix} 使用暴力BBR魔改版加速(可能无法使用)
